...
def train_one_run(args, run_id, fold_splits, identified, train_cases_list):
    ...
    for fold, (train_split, test_split) in enumerate(fold_splits):
        ...
        if args.task_type == 'survival':
            ...
            risk_all.append(np.array(temp_dict["risk"]).ravel())
            bag_fu_all.append(np.array(temp_dict["survival"]).ravel())
            bag_labels.append(np.array(temp_dict["event"]).ravel())
            patient_ids.append(np.array(temp_dict["case_id"]).ravel())
