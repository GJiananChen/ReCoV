...
def train_one_run(args, run_id, fold_splits, identified, train_cases_list):
    ...
    for fold, (train_split, test_split) in enumerate(fold_splits):
        ...
        if args.task_type == 'survival':
            ...
            temp_dict = {"risk":[],"survival":[],"event":[],"case_id":[]}
    ...
    return latest_val_cindex, patient_ids, risk_all, bag_fu_all, bag_labels
...
args.results_dir = os.path.join(args.results_dir, args.which_splits, args.param_code, str(args.exp_code) + '_s{}'.format(args.seed) + time.strftime("_%d%b_%H_%M", time.gmtime()))
if not os.path.isdir(args.results_dir):
...
settings.update({'experiment details':"Testing 0.5 default cindex due to 39 entities belonging to less than first event, rest no censoring and more epochs"})
...
if __name__ == "__main__":
    ...
    # patient_df = dataset.slide_data[["case_id","label","survival_months"]].drop_duplicates(keep="first")
    #Filter out cases less than first survival month
    # min_survival = dataset.slide_data.loc[dataset.slide_data["censorship"]==0,"survival_months"].min()
    # patient_df = patient_df.loc[patient_df["survival_months"]>=min_survival]
    #get case ids and the corresponding labels
    ...
    for seed in range(args.seed,args.seed+N_RUNS):
        latest_val_cindex, patient_ids, risk_all, bag_fu_all, bag_labels = train_one_run(args, seed, fold_splits, identified, train_cases_list)
        memory = rank_weights(latest_val_cindex,patient_ids, risk_all, bag_fu_all, bag_labels, memory)
